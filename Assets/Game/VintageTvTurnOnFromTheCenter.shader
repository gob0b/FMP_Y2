Shader "Custom/VintageTVTurnOnFromCenter"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // Mask to show the image expanding from the center (gradual reveal)
                float distToCenter = length(i.uv - float2(0.5, 0.5)); // Calculate distance from center
                float mask = smoothstep(0.0, 0.2, _Progress - distToCenter); // Gradually reveal from center

                // Apply curvature, flickering, and scanlines
                return fixed4(col.rgb + scanline + flicker, mask);
            }
            ENDCG
        }
    }
